...
class SAGM_DiffuseMix(torch.optim.Optimizer):
    ...
    @torch.no_grad()
    def set_closure(self, loss_fn, inputs_original, inputs_transformed, targets, **kwargs):
        ...
        # This function does not take any arguments except a flag to switch between original and transformed
        # inputs, because the inputs and targets data
        # should be pre-set in the definition of partial-function

        def get_grad(use_transformed: bool):
            self.base_optimizer.zero_grad()
            with torch.enable_grad():
                outputs = self.model(inputs_transformed if use_transformed else inputs_original)
    ...
    def step(self, closure=None):
        ...
        with self.maybe_no_sync():
            # get gradient
            outputs, loss_value = get_grad(use_transformed=False)
